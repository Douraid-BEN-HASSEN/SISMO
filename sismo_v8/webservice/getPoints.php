...
class CWebserviceGet
{
	...
	function read($option, $date, $heure, $seconde, $ms, $decalage)
	{
		if($option == 'all') $requette = "SELECT points, seconde, ms FROM `heure_$heure` WHERE `date` = '$date' ORDER BY seconde ASC, ms ASC";
		if($option == 'bigger') $requette = "SELECT points, seconde, ms FROM `heure_$heure` WHERE `date` = '$date' AND `seconde` >= $seconde AND `ms` > $ms ORDER BY seconde ASC, ms ASC";
		if($option == 'smaller') $requette = "SELECT points, seconde, ms FROM `heure_$heure` WHERE `date` = '$date' AND `seconde` < $seconde AND `ms` < $ms ORDER BY seconde ASC, ms ASC";
...
$ttt->read($option, $date, $heure, $seconde, $ms, $decalage);
